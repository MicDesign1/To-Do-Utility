...
//Rare exceptions to indicate a bug in the _todo code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_todo = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_todo]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.todo.module"
